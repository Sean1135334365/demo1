...
(function( $ ) {
    ...
    $.fn.extend({
		...
		*/
        diyUpload:function( opt, serverCallBack ) {
			...
			//迭代出默认配置
			$.each( getOption( '#'+$fileInputId , opt.addimg ),function( key, value ){
	...
	//Web Uploader默认配置;
	function getOption(objId,addimg) {
		/*
		...
		*/
		return {
			...
			pick:{
				id:objId,
				label:"选择图片",
				innerHTML:'<img src="'+addimg+'/Admin/assets/images/add.png" class="add_img" title="添加图片">'
			},
			//类型限制;
			accept:{
				title:"Images",
				extensions:"gif,jpg,jpeg,bmp,png",
				mimeTypes:"image/vnd.sealedmedia.softseal-jpg,image/jpeg,image/png,image/gif"
			},
			//配置生成缩略图的选项
			thumb:{
				width:119,
	...
	function createBox( $fileInput, file, webUploader ) {
		...
		if ( $parentFileBox.length <= 0 ) {
			...
			//$fileInput.after( div );//在按钮后面插入内容
			$fileInput.before( div );//在按钮前面插入内容
		...
		if ( $parentFileBox.find('.diyButton').length <= 0 ) {
			...
			var div = '<div class="diyButton"> \
						<a class="diyStart" href="javascript:void(0)">上&nbsp;传</a> \
						<a class="diyCancelAll" href="javascript:void(0)">取&nbsp;消</a> \
					</div>';
			$parentFileBox.append( div );

			var parent=$("#box").find('.parentFileBox');
			var parent1=$("#box1").find('.parentFileBox');

			var $startButton =parent.find('.diyStart');
			var $cancelButton = parent.find('.diyCancelAll');

            var $startButton1 =parent1.find('.diyStart');
            var $cancelButton1 = parent1.find('.diyCancelAll');
            ...
            $("#test").find(".webuploader-pick").html('' +
                ...
                $("#test").find(".webuploader-pick").css({
                 ...
                 if(parent.find(".fileBoxUl").children("li").length>=2){
                      parent.find(".fileBoxUl").children("li:first-child").remove();
                     $("#box").find(".parentFileBox").css({
                         "width":"119px",
                     })
                 }  else{
                     return
            ...
            var uploadStart1 = function (){
                ...
                if(parent1.find(".fileBoxUl").children("li").length>1){
                    parent1.find(".fileBoxUl").children("li:first-child").remove();
                    $("#box1").find(".parentFileBox").css({
                        "width":"119px",
                    })
                }  else{
                    return
                ...
                $startButton1.text('暂停上传').one('click',function(){
                    webUploader.stop();
                    $(this).text('继续上传').one('click',function(){
                        uploadStart1();
            ...
            $startButton1.one('click',uploadStart1);
			...
			$cancelButton.bind('click',function(){
                ...
                parent.find(".diyButton").remove()
                $("#box").find(".parentFileBox").css({
                    "width":"119px",
                })
			});

            $cancelButton1.bind('click',function(){
                var fileArr = webUploader.getFiles( 'queued' );
                $.each( fileArr ,function( i, v ){
                    removeLi( $('#fileBox_'+v.id), v.id, webUploader );
                });
                parent1.find(".diyButton").remove()
                $("#box1").find(".parentFileBox").css({
                    "width":"119px",
                })
            });
		...
		//父容器宽度;
		var $width = $('.fileBoxUl>li').length * 129;
